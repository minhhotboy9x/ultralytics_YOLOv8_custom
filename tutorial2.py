...
model = YOLO('yolov8s_coco10k.pt')

if __name__ == '__main__':
    # model.train(data = 'coco_minitrain_10k.yaml', epochs = 500, batch=16, device = 1) # run 5
    # model.train(data = 'coco_minitrain_10k.yaml', resume=True, batch=16, device = 1)
    metrics = model.val(data = 'coco_minitrain_10k.yaml', device = 0) # run 0
...
# python yolov8_KD/KD_training_ver7.py --model KD_feature/train10/weights/last.pt --teacher yolov8m_coco10k.pt --type_kd_loss dssim --resume True --device 1 # KD_feat 10
