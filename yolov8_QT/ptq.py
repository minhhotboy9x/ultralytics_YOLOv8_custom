...
from utils import replace_conv_with_qconv_v2
...
def calibration(model, args):
    ...
    for i, batch in pbar:
        batch = preprocess_batch(batch, args.device)
        model(batch['img'])
...
def main(args):
    ...
    # print(model)
    replace_conv_with_qconv_v2(detect_model)
    ...
    metrics = model.val(data=args.data, batch=args.batch, device=args.device)
    print('-----------------------')
    model.info()
    print(metrics)
    model.export()
